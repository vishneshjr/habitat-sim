...
class DebugVisualizer:
    ...
    def look_at(
        ...
        camera_pos = (
            look_from if look_from is not None else agent.scene_node.translation
        )
        if look_up is None:
    ...
    def _peek_object(
        ...
        if not peek_all_axis:
            ...
            for axis in range(6):
                ...
                look_from = (
                    obj_abs_transform.transform_vector(axis_vec).normalized() * distance
                    + look_at
                )
...
class Receptacle:
    ...
    def __init__(
        ...
        name: str,
        bounds: mn.Range3D,
        up: Optional[mn.Vector3] = None,  # used for culling, optional in config
